...
document.querySelector(".buttons").addEventListener("mousemove", (e) => {
    ...
    document.querySelector(
        ".buttons"
    ).style.backgroundImage = `radial-gradient(circle at ${e.clientX}px ${
        e.clientY - top
    }px, #00aff577 0, transparent 48px)`;
});
...
document.getElementById("openInNewWindow").addEventListener("click", openPopup);

function openPopup() {
    let chromeWidthPadding = 32;
    let chromeHeightPadding = 40; // 67 for Opera, 40 for others
    if (navigator.userAgent.includes("OPR")) {
        chromeHeightPadding = 67;
    }
    let width = Math.ceil(document.body.scrollWidth) + chromeWidthPadding;
    let height = Math.ceil(document.body.scrollHeight) + chromeHeightPadding;
